...
const PHASE_B_IDS = ["G02", "G12", "G13"] as const;
...
const phaseBCases = golden.cases.filter((row) =>
  (PHASE_B_IDS as readonly string[]).includes(row.id),
);
...
function assertInvestigateExpect(pkg: EvidencePackage, row: GoldenCase): void {
  ...
  if (expected.packages) {
    assertPackages(pkg, expected.packages);
  }
}
...
describe("Phase B golden runner", () => {
  ...
  it("loads only the Phase B golden cases", () => {
    expect(golden.phase_b).toEqual([...PHASE_B_IDS]);
    expect(phaseBCases.map((row) => row.id)).toEqual([...PHASE_B_IDS]);
  });

  it.each(phaseBCases)("$id — $question", async (row) => {
