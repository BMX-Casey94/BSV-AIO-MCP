...
- [conformance/vectors/sdk/keys/key-derivation.json](conformance/vectors/sdk/keys/key-derivation.json)
- [conformance/vectors/sdk/keys/private-key.json](conformance/vectors/sdk/keys/private-key.json)
- [conformance/vectors/sdk/keys/public-key.json](conformance/vectors/sdk/keys/public-key.json)
...
graph TD
    ...
    CE -->|"ECDSA.verify()"| BE
```
**Sources:** [conformance/vectors/sdk/keys/private-key.json:7-7](), [conformance/vectors/sdk/keys/public-key.json:7-7](), [conformance/vectors/sdk/crypto/ecdsa.json:7-7]()
...
**Sources:** [conformance/vectors/sdk/crypto/ecdsa.json:115-140]()
...
**Sources:** [conformance/vectors/sdk/crypto/sha256.json:3-8](), [conformance/vectors/sdk/crypto/ripemd160.json:3-8](), [conformance/vectors/sdk/crypto/hash160.json:3-8](), [conformance/vectors/sdk/crypto/hmac.json:3-8]()
...
*   **Deterministic DRBG:** Uses RFC 6979 for deterministic $k$ generation to prevent nonce reuse [conformance/vectors/sdk/crypto/ecdsa.json:12-24]().
*   **Low-S Normalization:** Ensures signatures use the lower $s$ value to prevent malleability [conformance/vectors/sdk/crypto/ecdsa.json:71-84]().
*   **Point at Infinity Protection:** Explicitly rejects the point at infinity as a valid public key [conformance/vectors/sdk/crypto/ecdsa.json:185-197]().

### Schnorr Signatures
Implemented for use in modern BSV smart contracts and BRC-standardized authentication.

### Key Management
*   **PrivateKey:** Supports WIF (Wallet Import Format) decoding and encoding [conformance/vectors/sdk/keys/private-key.json:59-69]().
*   **PublicKey:** Supports both compressed (33 bytes) and uncompressed (65 bytes) formats [conformance/vectors/sdk/keys/public-key.json:10-40]().

**Sources:** [conformance/vectors/sdk/crypto/ecdsa.json:1-10](), [conformance/vectors/sdk/keys/private-key.json:1-9]()
...
*   **Key Sizes:** Supports 128-bit, 192-bit, and 256-bit keys [conformance/vectors/sdk/crypto/aes.json:11-42]().
*   **Authentication:** Produces a 16-byte authentication tag to ensure data integrity [conformance/vectors/sdk/crypto/aes.json:52-55]().

**Sources:** [conformance/vectors/sdk/crypto/aes.json:1-9]()
...
The derivation uses the recipient's private key, the sender's public key, and an `invoiceNumber` (string or buffer) to produce a unique child key [conformance/vectors/sdk/keys/private-key.json:83-146]().

**Sources:** [conformance/vectors/sdk/keys/private-key.json:83-89](), [conformance/vectors/sdk/keys/key-derivation.json:1-10]()
...
*   **Vector Runner:** The `conformance-runner` executes tests against JSON-defined vectors [conformance/runner/src/runner.js:1-15]().
*   **Parity Classes:** Tests are categorized as `required`, `crypto`, or `best-effort` to ensure core functionality is never compromised [conformance/vectors/sdk/crypto/ecdsa.json:8-8]().

**Sources:** [conformance/runner/package.json:1-10](), [conformance/runner/src/runner.js:78-81]()
...
- [conformance/vectors/sdk/transactions/merkle-path.json](conformance/vectors/sdk/transactions/merkle-path.json)
- [conformance/vectors/sdk/transactions/serialization.json](conformance/vectors/sdk/transactions/serialization.json)
...
*   **Shift Operations**: `OP_LSHIFT` and `OP_RSHIFT` must truncate results to the original byte length and preserve input endianness [conformance/vectors/regressions/script-lshift-truncation.json:5-11](), [conformance/vectors/regressions/script-shift-endianness.json:5-11]().
*   **ASM Parsing**: `Script.fromASM()` treats bare hex strings (like '76') as data pushes rather than opcodes when they appear in a data context [conformance/vectors/regressions/script-fromasm-numeric-token.json:5-11]().
*   **Empty Pushes**: `writeBin([])` correctly produces an `OP_0` (0x00) rather than an empty string [conformance/vectors/regressions/script-writebin-empty.json:5-11]().
...
BUMP provides a compact way to represent Merkle paths. The `MerklePath` class handles the computation of missing hashes, especially in trees with an odd number of nodes where duplication logic is critical [conformance/vectors/regressions/merkle-path-odd-node.json:5-11]().
...
**BEEF Class Logic**
The `Beef` class manages the hydration of source transactions. When `Beef.IsValid(true)` is called, it must back-link `input.SourceTransaction` pointers from its internal transactions map to ensure the full graph can be validated [conformance/vectors/regressions/beef-isvalid-hydration.json:5-11]().
...
A minimum of 1 satoshi is applied for any non-zero transaction size when the rate is positive [conformance/vectors/regressions/fee-model-mismatch.json:5-11]().
...
A critical fix in the SDK ensures that if an input is constructed with `sequence = 0`, the `Sighash` preimage correctly uses `0x00000000` rather than defaulting to `0xFFFFFFFF`, which would invalidate the signature [conformance/vectors/regressions/tx-sequence-zero-sighash.json:5-11]().
...
- [packages/wallet/ts-wallet-relay/package.json](packages/wallet/ts-wallet-relay/package.json)
...
graph TD
    ...
    J -- "Pairs" --> A
```
Sources: [packages/wallet/wallet-toolbox/package.json:2-4](), [packages/wallet/wab/package.json:2-5](), [packages/wallet/ts-wallet-relay/package.json:2-4](), [packages/wallet/btms/src/BTMS.ts:98-120]()

---

## Component Overview

### [wallet-toolbox: Storage, Signer & Wallet Manager](#3.1)
...
*   **For details, see [wallet-toolbox: Storage, Signer & Wallet Manager](#3.1).**

Sources: [packages/wallet/wallet-toolbox/package.json:41-52](), [packages/wallet/wallet-toolbox/package.json:2-4]()

### [WAB: Wallet Authentication Backend](#3.2)
...
*   **For details, see [WAB: Wallet Authentication Backend](#3.2).**

Sources: [packages/wallet/wab/package.json:15-26](), [packages/wallet/wab/package.json:2-5]()

### [Wallet Relay & Mobile Pairing](#3.3)
...
*   **For details, see [Wallet Relay & Mobile Pairing](#3.3).**

Sources: [packages/wallet/ts-wallet-relay/package.json:6-22](), [packages/wallet/ts-wallet-relay/package.json:40-46]()

### [BTMS: Token Management System](#3.4)
...
*   **For details, see [BTMS: Token Management System](#3.4).**

Sources: [packages/wallet/btms/src/BTMS.ts:98-134](), [packages/wallet/btms/README.md:78-87]()
...
| :--- | :--- | :--- |
| `@bsv/chaintracks-server` | Tracks blockchain headers for SPV validation. | [packages/network/chaintracks-server/package.json:5-6]() |
| `@bsv/message-box-client` | Asynchronous delivery of tokens and payment requests. | [packages/wallet/btms/src/BTMS.ts:38-43]() |
| `@bsv/auth-express-middleware` | BRC-103 mutual authentication for wallet services. | [packages/wallet/wallet-toolbox/package.json:42]() |
...
sequenceDiagram
    ...
    BTMS-->>App: SendResult
```
Sources: [packages/wallet/btms/src/BTMS.ts:161-230](), [packages/wallet/btms/README.md:57-61]()
...
- [packages/messaging/messagebox-services/backend/package-lock.json](packages/messaging/messagebox-services/backend/package-lock.json)
- [packages/wallet/ts-wallet-relay/dist/client.cjs.map](packages/wallet/ts-wallet-relay/dist/client.cjs.map)
- [packages/wallet/ts-wallet-relay/dist/client.js.map](packages/wallet/ts-wallet-relay/dist/client.js.map)
- [packages/wallet/ts-wallet-relay/dist/index.cjs.map](packages/wallet/ts-wallet-relay/dist/index.cjs.map)
- [packages/wallet/ts-wallet-relay/dist/index.js.map](packages/wallet/ts-wallet-relay/dist/index.js.map)
- [packages/wallet/ts-wallet-relay/dist/react.cjs.map](packages/wallet/ts-wallet-relay/dist/react.cjs.map)
- [packages/wallet/ts-wallet-relay/dist/react.js.map](packages/wallet/ts-wallet-relay/dist/react.js.map)
- [packages/wallet/ts-wallet-relay/package.json](packages/wallet/ts-wallet-relay/package.json)
...
```
Sources: `[packages/wallet/ts-wallet-relay/dist/index.js.map:1-1]()`, `[packages/wallet/ts-wallet-relay/dist/client.js.map:1-1]()`
...
### Key Components
-   **`WebSocketRelay`**: Manages raw WebSocket connections, heartbeats, and message buffering for disconnected clients `[packages/wallet/ts-wallet-relay/dist/index.js.map:39-40]()`.
-   **`QRSessionManager`**: Handles the lifecycle of pairing sessions, including generating the `pairingUri` and QR codes `[packages/wallet/ts-wallet-relay/dist/index.js.map:1-1]()`.
-   **`WalletRequestHandler`**: An Express-compatible handler for the `/session` and `/session/:topic` HTTP endpoints `[packages/wallet/ts-wallet-relay/dist/index.js.map:1-1]()`.

### WebSocket Relay Logic
The relay uses a `TopicEntry` to track the two halves of a pairing. If one side is disconnected when a message arrives, the relay buffers the message (up to `BUFFER_MAX_PER_TOPIC`) for `BUFFER_TTL_MS` (60 seconds) `[packages/wallet/ts-wallet-relay/dist/index.js.map:5-7]()`.

| Feature | Implementation Detail |
| :--- | :--- |
| **Heartbeat** | Every 30s (`HEARTBEAT_INTERVAL_MS`) to keep connections alive `[packages/wallet/ts-wallet-relay/dist/index.js.map:5-5]()`. |
| **Role Validation** | Rejects connections missing `topic` or `role` (desktop/mobile) `[packages/wallet/ts-wallet-relay/dist/index.js.map:135-145]()`. |
| **Origin Security** | Validates `Origin` headers for `role=desktop` browser clients `[packages/wallet/ts-wallet-relay/dist/index.js.map:150-155]()`. |

Sources: `[packages/wallet/ts-wallet-relay/dist/index.js.map:1-170]()`
...
### Implementation Details
-   **`connect()`**: Establishes the WebSocket connection with `role=mobile` `[packages/wallet/ts-wallet-relay/dist/client.js.map:84-84]()`.
-   **`onRequest(handler)`**: Registers a callback to process decrypted RPC methods (e.g., `getPublicKey`, `signAction`) `[packages/wallet/ts-wallet-relay/dist/client.js.map:127-130]()`.
-   **Replay Protection**: Tracks a sequence number (`_lastSeq`) to prevent message replay attacks `[packages/wallet/ts-wallet-relay/dist/client.js.map:87-87]()`.

### Method Handling
The session distinguishes between methods that require user interaction and those that can be auto-approved:
-   **`DEFAULT_IMPLEMENTED_METHODS`**: The standard set of BRC-100 methods supported by the BSV mobile ecosystem `[packages/wallet/ts-wallet-relay/dist/client.js.map:11-16]()`.
-   **`DEFAULT_AUTO_APPROVE_METHODS`**: Includes `getPublicKey` by default `[packages/wallet/ts-wallet-relay/dist/client.js.map:22-22]()`.

Sources: `[packages/wallet/ts-wallet-relay/dist/client.js.map:1-135]()`
...
-   **`session`**: Contains the current connection status (`idle`, `connecting`, `connected`, `disconnected`) and pairing metadata `[packages/wallet/ts-wallet-relay/dist/react.js.map:30-30]()`.
-   **`sendRequest(method, params)`**: Sends an encrypted RPC request to the mobile wallet `[packages/wallet/ts-wallet-relay/dist/react.js.map:61-64]()`.
-   **`wallet`**: A proxy object that implements the `WalletInterface`, allowing the app to call `wallet.getPublicKey()` directly when connected `[packages/wallet/ts-wallet-relay/dist/react.js.map:85-87]()`.

### UI Components
-   **`WalletConnectionModal`**: A pre-built modal that displays the QR code and connection status `[packages/wallet/ts-wallet-relay/dist/react.cjs.map:15-16]()`.
-   **`QRPairingCode`**: A tappable QR code component. On mobile browsers, tapping it triggers the `wallet://pair` deeplink `[packages/wallet/ts-wallet-relay/dist/react.js.map:32-38]()`.
...
```
Sources: `[packages/wallet/ts-wallet-relay/dist/react.js.map:1-90]()`, `[packages/wallet/ts-wallet-relay/dist/react.cjs.map:1-75]()`
...
4.  **Persistence**: The `WalletRelayClient` can persist session tokens in `localStorage` `[packages/wallet/ts-wallet-relay/dist/react.js.map:42-51]()`.
5.  **Resumption**: Upon page reload, `resumeSession()` attempts to reconnect to the existing topic using the stored token `[packages/wallet/ts-wallet-relay/dist/react.js.map:73-76]()`.
...
Sources: `[packages/wallet/ts-wallet-relay/dist/index.js.map:1-25]()`, `[packages/wallet/ts-wallet-relay/dist/react.js.map:70-80]()`
...
1.  **Phase 1: Initial Handshake**: The client calls `POST /.well-known/auth`.
    *   **Client Request**: Sends an `initialRequest` containing a fresh nonce signed by the client's identity key [specs/auth/brc31-handshake.yaml:29-37]().
    *   **Server Response**: Validates the client, generates its own nonce, signs the response, and returns an `initialResponse`. It may also include a `requestedCertificates` field for BRC-52 selective disclosure [specs/auth/brc31-handshake.yaml:39-50]().
2.  **Phase 2: Authenticated Requests**: Subsequent requests carry `x-bsv-auth-*` headers.
    *   The client signs the request metadata (method, path, headers, body) and the server's previous nonce [specs/auth/brc31-handshake.yaml:54-65]().
    *   The server responds with its own signature over the response metadata, allowing the client to authenticate the server [specs/auth/brc31-handshake.yaml:67-78]().
...
```
Sources: [specs/auth/brc31-handshake.yaml:10-15](), [specs/messaging/message-box-http.yaml:7-13]()

### Key Implementation Details
*   **Response Interception**: The `ExpressTransport` intercepts standard Express response methods (`res.json`, `res.send`, `res.status`) to buffer the response. This allows the middleware to sign the final outgoing payload before it is sent to the client [specs/auth/brc31-handshake.yaml:95-98]().
*   **Replay Protection**: The `SessionManager` (within the SDK) tracks seen nonces. Nonces are single-use; if a nonce is reused, the request is rejected [specs/auth/brc31-handshake.yaml:100-101]().
*   **Unauthenticated Pass-through**: If `allowUnauthenticated: true` is configured, requests without headers proceed but are assigned an identity key of `'unknown'` [specs/auth/brc31-handshake.yaml:90-91]().

Sources: [specs/auth/brc31-handshake.yaml:90-101]()
...
When a client sends a message that requires a fee (e.g., `recipientFee` or `deliveryFee`), it attaches a `Payment` object [specs/messaging/message-box-http.yaml:184-190]().
...
```
Sources: [specs/messaging/message-box-http.yaml:139-167](), [specs/messaging/message-box-http.yaml:184-200]()

### Key Components
*   **Atomic BEEF**: Payments are transmitted using the Background Evaluation Extended Format (BEEF). This allows the server to validate the transaction's provenance without querying a centralized indexer [specs/messaging/message-box-http.yaml:61-67]().
*   **Remittance Metadata**: Each payment output includes a `paymentRemittance` object containing `derivationPrefix` and `derivationSuffix`. These are used by the receiver to derive the specific public key used for the output, following BRC-42 [specs/messaging/message-box-http.yaml:151-161]().
*   **Routing Instructions**: The middleware can parse `customInstructions` within the remittance. If a `recipientIdentityKey` is present, the payment is routed to that specific recipient in multi-party scenarios [specs/messaging/message-box-http.yaml:162-167]().

Sources: [specs/messaging/message-box-http.yaml:139-200](), [specs/payments/brc29-payment-protocol.yaml:76-77]()
...
| :--- | :--- | :--- |
| **Authentication** | `@bsv/auth-express-middleware` | `x-bsv-auth-identity-key` [specs/messaging/message-box-http.yaml:35]() |
| **Message Delivery** | N/A | `POST /sendMessage` [specs/messaging/message-box-http.yaml:237]() |
| **Payment** | `@bsv/payment-express-middleware` | `Payment` object in body [specs/messaging/message-box-http.yaml:184]() |

Sources: [specs/messaging/message-box-http.yaml:7-15](), [specs/messaging/message-box-http.yaml:32-41]()
